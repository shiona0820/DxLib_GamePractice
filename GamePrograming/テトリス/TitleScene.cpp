...
*タイトル画面：初期化処理
* 引数：なし
* 戻り値エラー情報(-1異常有、-1以外：正常終了）
**/
int TItleScene_Initialaize(void)
{
	int ret = 0;
	...
	sounds[E_TITLE_SE_CURSOR = LoadSoundMem("sounds/SE2.mp3");

	ChangeVolumeSoundMem(120, sounds[E_TITLE_SE_CURSOR]);
	ChangVolumeSoundMem(80, sounds[E_TITLE_SE_SELECT]);
...
{
	//bgmが流れていないときに再生
	if (ChackSoundMem(sounds[E_TITLE_BGM]) != TRUE)
	{
